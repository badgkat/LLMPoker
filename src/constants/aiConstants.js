...
export function calculate4DBehaviorThresholds(strategy) {
  ...
  // Convert 4D ratings to actionable thresholds
  const handSelectionThreshold = 0.2 + (tightness * 0.6); // 0.2-0.8 range
  const raiseFrequency = 0.1 + (aggression * 0.4); // 0.1-0.5 range
  const bluffFrequency = (aggression * 0.3) + (riskTolerance * 0.2); // 0.0-0.5 range
  const foldThreshold = 0.3 + (tightness * 0.4); // 0.3-0.7 range
